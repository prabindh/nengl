...
void init_object(nengl_core* obj1, unsigned int textureId, int texture_w, int texture_h)
{
	...
	obj1->setup_fbo_as_texture_data(&texture[0], textureId);
...
int main()
{
    ...
    {
        /* STAGE 1: Draw to PING texture */	
        //setup the object obj1 first time, with no input texture
        nengl_core* obj1 = new nengl_core();
	nengl_fbo* fbo=new nengl_fbo(256, 256);
        init_object(obj1, 0, 0, 0);
	...
	obj1->draw();
        /* STAGE 2: Set PING texture as input and draw to display */
	fbo->unset();
  	delete obj1;
        //setup the object obj1 second time, with fbo input texture
        obj1 = new nengl_core();
        init_object(obj1, fbo->get_reference_id(), fbo->get_width(), fbo->get_height());
	//Clear with different color this time
        ws->clear_screen(1.0, 0.0, 0.0);
        ws->clear_depth();
        /* Rotate obj1 */
        obj1->default_transform(NENGL_VIEW_PERSPECTIVE);
        obj1->translate(0, 0, 5, "mvMatrix");
