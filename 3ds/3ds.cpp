...
bool CLoad3DS::Import3DS(t3DModel *pModel, char *strFileName)
{
	...
	//Prabu added
	pModel->numOfVertices = pModel->numOfIndices = 0;
...
D_PRINTF("Chunk ID = %x\n", m_CurrentChunk->ID);
		...
		case VERSION:							// This holds the version of the file
			...
			if (version > 0x03)
				D_PRINTF("This 3DS file is over version 3 so it may load incorrectly\n");
			break;

		case OBJECTINFO:						// This holds the version of the mesh

#if 0 //Prabu added - No version of Mesh in Blender output. Enabling this will cause material chunks to be NOT read !
		...
		case MATERIAL:							// This holds the material information
			...
			D_PRINTF("Materials incremented to %d\n", pModel->numOfMaterials);
		...
		case OBJECT:							// This holds the name of the object being read
			...
			D_PRINTF("Name = %s\n", pModel->pObject[pModel->numOfObjects - 1].strName);
...
#else
			tempCount = (m_CurrentChunk->length - m_CurrentChunk->bytesRead);
			m_CurrentChunk->bytesRead += tempCount;
			fseek(m_FilePointer, tempCount, SEEK_CUR);
#endif
			break;
...
{
	//#define MAX_UNWANTED_SIZE (50000)
	//int buffer[MAX_UNWANTED_SIZE] = {0};					// This is used to read past unwanted data
	...
	while (pPreviousChunk->bytesRead < pPreviousChunk->length)
	{
		...
		case OBJECT_VERTICES:				// This is the objects vertices
			...
			//Prabu added
			pModel->numOfVertices += pObject->numOfVerts;
			break;

		case OBJECT_FACES:					// This is the objects face information
			ReadVertexIndices(pObject, m_CurrentChunk);
			pModel->numOfIndices += (3*pObject->numOfFaces);
			break;
...
#if 0
			//PS added - check
			if((m_CurrentChunk->length - m_CurrentChunk->bytesRead) > MAX_UNWANTED_SIZE)
			{
				D_PRINTF("Error skipping unknown chunks due to memory!\n");
			}
			//Prabu modified to remove buffer allocation
			//m_CurrentChunk->bytesRead += fread(buffer, 1, m_CurrentChunk->length - m_CurrentChunk->bytesRead, m_FilePointer);
#else
			int tempCount = (m_CurrentChunk->length - m_CurrentChunk->bytesRead);
			m_CurrentChunk->bytesRead += tempCount;
			fseek(m_FilePointer, tempCount, SEEK_CUR);
#endif
			break;
...
//Prabu added
	pObject->indices = new unsigned short [3*pObject->numOfFaces];
	...
	for(int i = 0; i < pObject->numOfFaces; i++)
	{
		...
		for(int j = 0; j < 4; j++)
		{
			...
			if(j < 3)
			{
				...
				//Prabu added
				pObject->indices[i*3 + j] = (unsigned short) index;
			}
		}
	}
...
void CLoad3DS::ReadUVCoordinates(t3DObject *pObject, tChunk *pPreviousChunk)
{
	...
	int toRead = pPreviousChunk->length - pPreviousChunk->bytesRead; 
	pPreviousChunk->bytesRead += fread(pObject->pTexVerts, 1, toRead, m_FilePointer);

	//Prabu added
#if 1
	pObject->uvcoords = (float*)pObject->pTexVerts;
#else
	pObject->uvcoords = new float [2*pObject->numTexVertex];
	memcpy(pObject->uvcoords, pObject->pTexVerts, 2*sizeof(float)*pObject->numTexVertex);
	if(toRead != (2*sizeof(float)*pObject->numTexVertex))
	{
		D_PRINTF("Error - mismatch in ReadUVCoordinates (%d, %d)\n", toRead,(2*sizeof(float)*pObject->numTexVertex));
	}
#endif
}
...
//Prabu added
#if 0
	pObject->vertices = new float [3*pObject->numOfVerts];
#endif
...
#if 1
		pObject->vertices = (float*)pObject->pVerts;
#else
		//Prabu added
		pObject->vertices[i*3] = pObject->pVerts[i].x;
		pObject->vertices[i*3 + 1] = pObject->pVerts[i].y;
		pObject->vertices[i*3 + 2] = pObject->pVerts[i].z;
#endif
	}
}
...
//static int tempMaterialbuffer[50000] = {0};				// This is used to read past unwanted data
void CLoad3DS::ReadObjectMaterial(t3DModel *pModel, t3DObject *pObject, tChunk *pPreviousChunk)
{
	...
	D_PRINTF("number of materials in Model = %d\n", pModel->numOfMaterials);
	pObject->texture_filename = 0;
	for(int i = 0; i < pModel->numOfMaterials; i++)
	{
		// If the material we just read in matches the current texture name
		if(strcmp(strMaterial, pModel->pMaterials[i].strName) == 0)
		{
			D_PRINTF("Material file name for %s = %s\n", strMaterial, pModel->pMaterials[i].strFile);
			...
			if(strlen(pModel->pMaterials[i].strFile) > 0) {
				...
				//Prabu added
				pObject->texture_filename = pModel->pMaterials[i].strFile;
			}	
			break;
		}
		else
		{
			D_PRINTF("No material found with name - %s\n", strMaterial);
...
#if 0
	pPreviousChunk->bytesRead += fread(tempMaterialbuffer, 1, pPreviousChunk->length - pPreviousChunk->bytesRead, m_FilePointer);
#else
	int tempCount = (pPreviousChunk->length - pPreviousChunk->bytesRead);
	pPreviousChunk->bytesRead += tempCount;
	fseek(m_FilePointer, tempCount, SEEK_CUR);
#endif
...
void CLoad3DS::ComputeNormals(t3DModel *pModel)
{
	...
	for(int index = 0; index < pModel->numOfObjects; index++)
	{
		...
		D_PRINTF("Name = %s, Faces = %d\n", pObject->strName, pObject->numOfFaces);
